...
static GFont          *customTIME;
static GFont          *customDATE;
...
static void handle_battery(BatteryChargeState charge_state) {
  ...
  inv_layer = inverter_layer_create(GRect(51, 148, (charge_state.charge_percent/10)*2, 9));
...
static void handle_second_tick(struct tm* tick_time, TimeUnits units_changed) {
  ...
  static char date_text[] = "September 99th";
  static char dp[] = "18";
...
switch(tick_time->tm_mday)
  {
    ...
    case 21 :
    case 31 :
      strftime(date_text, sizeof(date_text), "%A %est", tick_time);
      break;
    case 2 :
    case 22 :
      strftime(date_text, sizeof(date_text), "%A %end", tick_time);
      break;
    case 3 :
    case 23 :
      strftime(date_text, sizeof(date_text), "%A %erd", tick_time);
      break;
    default :
      strftime(date_text, sizeof(date_text), "%A %eth", tick_time);
      break;
  }
...
static void do_init(void) {
  ...
  customTIME  = fonts_load_custom_font( resource_get_handle( RESOURCE_ID_FONT_ROBOTO_56 ) );
  customDATE  = fonts_load_custom_font( resource_get_handle( RESOURCE_ID_FONT_ROBOTO_18 ) );
  ...
  /* Time block */
  time_layer = text_layer_create(GRect(0, 65, 144, 60));
  text_layer_set_text_color(time_layer, GColorWhite);
  text_layer_set_background_color(time_layer, GColorClear);
  text_layer_set_font(time_layer, customTIME);
  text_layer_set_text_alignment(time_layer, GTextAlignmentCenter);
	
  /* Date block */
  date_layer = text_layer_create(GRect(0, 121, 144, 30));
  text_layer_set_text_color(date_layer, GColorBlack);
  text_layer_set_background_color(date_layer, GColorClear);
  text_layer_set_font(date_layer, customDATE);
  text_layer_set_text_alignment(date_layer, GTextAlignmentCenter);

  /* Bluetooth block */
  bt_disconnected = gbitmap_create_with_resource(RESOURCE_ID_BT_DISCONNECTED);
  bt_connected = gbitmap_create_with_resource(RESOURCE_ID_BT_CONNECTED);
  bt_connected_layer = bitmap_layer_create(GRect(0,0,144,81));
  ...
  /* Battery block */
  battery_layer = text_layer_create(GRect(19, 143, 144, 30 ));
  text_layer_set_text_color(battery_layer, GColorBlack);
...
static void do_deinit(void) {
  ...
  fonts_unload_custom_font(customTIME);
  fonts_unload_custom_font(customDATE);
